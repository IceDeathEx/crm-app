...
async function getOrCreateUser(session) {
  ...
  try {
    ...
    // Concurrent requests (e.g. browser prefetch) can race to create the
    // same user; if another request won, just return what it created.
    if (err.code === "P2002") {
      return db.user.findUniqueOrThrow({ where: { auth0Sub: sub } });
    }
    throw err;
  }
}
